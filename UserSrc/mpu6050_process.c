...
MPU6050_QUATERNION_STRUCT quat = {1.0f, 0.0f, 0.0f, 0.0f};
/** quaternion of sensor frame relative to auxiliary frame */
MPU6050_QUATERNION_STRUCT dq = {0.0f};
/** quaternion of sensor frame relative to auxiliary frame */
...
static void NonlinearSO3AHRSinit(float ax, float ay, float az, float mx, float my, float mz)
{
    ...
    quat.q0 = cosRoll * cosPitch * cosHeading + sinRoll * sinPitch * sinHeading;
    quat.q1 = sinRoll * cosPitch * cosHeading - cosRoll * sinPitch * sinHeading;
    quat.q2 = cosRoll * sinPitch * cosHeading + sinRoll * cosPitch * sinHeading;
    quat.q3 = cosRoll * cosPitch * sinHeading - sinRoll * sinPitch * cosHeading;
...
static void NonlinearSO3AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float twoKp, float twoKi, float dt)
{
    ...
    dq.q0 = 0.5f*(-quat.q1 * gx - quat.q2 * gy - quat.q3 * gz);
    dq.q1 = 0.5f*(+quat.q0 * gx + quat.q2 * gz - quat.q3 * gy);
    dq.q2 = 0.5f*(+quat.q0 * gy - quat.q1 * gz + quat.q3 * gx);
    dq.q3 = 0.5f*(+quat.q0 * gz + quat.q1 * gy - quat.q2 * gx);
...
//          startTime=now;
//
//      gyro_offsets_sum[0] += MPU6050_DATA.gyr_x;
//      gyro_offsets_sum[1] += MPU6050_DATA.gyr_y;
//      gyro_offsets_sum[2] += MPU6050_DATA.gyr_z;
//      offset_count++;
//
